...
$genbaName = $_GET['genba_name'] ?? '';
$disabled = $_GET['disabled'] ?? false;
...
<body>
        ...
        <form action="submit_inspection.php" method="POST">
            ...
            <div class="col-md-4 mb-2">
                <div class="form-group">
                    <label for="genbaSelect">現場</label>
                    <input type="text" class="form-control" value="<?= htmlspecialchars($genbaName) ?>" readonly>
                    <input type="hidden" name="genba_id" value="<?= htmlspecialchars($genbaId) ?>">
                </div>
            </div>
    ...
    <script>
                ...
                .then(data => {
                    if (data.status === 'success') {
                        ...
                        if (modal) {
                            ...
                            // モーダルインスタンスを取得できない場合の処理を追加
                            console.error("モーダルインスタンスを取得できませんでした。モーダルのIDが正しいか確認してください。");
                            // 例えば、モーダルを閉じるための別の方法を試す
        ...
        document.getElementById('inspectionForm').addEventListener('submit', function(e) {
            ...
            fetch('submit_inspection.php', {
                    method: 'POST',
                    body: formData
                })
                .then(response => response.json())
                .then(data => {
                    if (data.status === 'success') { // レスポンスのstatusを確認
                        ...
                        alert('エラー: ' + data.message); // エラーメッセージを表示
                    }
                })
                .catch(error => {
