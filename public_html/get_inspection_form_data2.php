...
// 点検対象の取得 (例: inspection_targets テーブルから)
// エラー原因: テーブル名 '試作2.inspection_targets' が存在しない
// テーブル名を 'inspection_target_masters' に修正 (テーブル名が異なる可能性があるため)
$targetsSql = "SELECT name FROM inspection_types WHERE type_id = ?"; // テーブル名とカラム名を調整
$targetsStmt = $conn->prepare($targetsSql);
$targetsStmt->bind_param('i', $inspection_type_id);
$targetsStmt->execute();
$targetsResult = $targetsStmt->get_result();

$targets = [];
while ($row = $targetsResult->fetch_assoc()) {
    $targets[] = ['name' => $row['name']]; // 'target_name' はテーブルのカラム名に合わせてください
}
$targetsStmt->close();
