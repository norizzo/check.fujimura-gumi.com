...
if (php_sapi_name() !== 'cli') {
    ...
    echo "<!DOCTYPE html><html><head><meta charset='utf-8'><title>target_name正規化</title></head><body><pre>";
}
...
echo "=== target_name テーブル正規化処理開始 ===\n\n";

$result = $conn->query("SELECT id, name FROM target_name");
if (!$result) {
...
while ($row = $result->fetch_assoc()) {
    ...
    if ($newName !== $originalName) {
        $stmt = $conn->prepare("UPDATE target_name SET name = ? WHERE id = ?");
