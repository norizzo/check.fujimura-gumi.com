                    ...
                    <label for="genbaSelect" class="form-label">現場名を選択してください</label>
                        <select id="genbaSelect" name="genba_id" class="form-select" onchange="changeColorGenba(this)" required>
                            ...
                            <?php while ($row = $genbaResult->fetch_assoc()): ?>
                                <?php 
                                    // デバッグ出力を追加
                                    // echo "<script>console.log('現場名:', " . json_encode($row['genba_name']) . ");</script>";
                                    // echo "<script>console.log('filteredData:', " . json_encode($filteredData) . ");</script>";
                                    // echo "<script>console.log('filteredData[現場名]が存在するか:', " . json_encode(isset($filteredData[$row['genba_name']])) . ");</script>";
                                ?>
    ...
    <script>
        ...
        function updateButtons() {
            ...
            fetch(`get_inspection_status.php?genba_id=${selectedGenbaId}&date=<?php echo $today; ?>`)
                .then(response => {
                    if (!response.ok) {
                        console.error('get_inspection_status.php からのデータ取得エラー: HTTPステータス', response.status);
